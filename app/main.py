...
from .core.auth import CurrentUser, current_user_dependency
from .services.pdf_service import PDFService
from .services.nlp_service import NLPService
from .services.ai_service import AIService
from .services.db_service import DBService
from .services.scoring_service import ScoringService
from .models.schemas import Flashcard, FlashcardUpdate, Deck, User
...
async def upload_pdf(
    ...
    print(f"DEBUG: Processing upload for user {current_user.user_id}, Deck: {deck_id}", flush=True)
...
async def analyze_pdf(
    ...
    try:
        ...
        print(f"DEBUG analyze error: {e!s}", flush=True)
        raise HTTPException(
            status_code=400,
            detail=f"PDF işlenemedi. Geçerli bir PDF olduğundan emin olun. ({str(e)[:200]})",
        ) from e
...
async def get_flashcards(
    ...
    allowed_decks = {d.deck_id for d in db.get_user_decks(current_user.user_id)}
    if deck_id not in allowed_decks:
        raise HTTPException(status_code=403, detail="Bu deste için erişim yetkiniz yok")
...
async def answer_card(
    ...
    allowed_decks = {d.deck_id for d in db.get_user_decks(current_user.user_id)}
    if card.deck_id not in allowed_decks:
        raise HTTPException(status_code=403, detail="Bu kart için erişim yetkiniz yok")
...
@app.post("/auth/sync", response_model=User)
async def sync_current_user(
    db: DBService = Depends(get_db_service),
    current_user: CurrentUser = Depends(current_user_dependency),
):
    existing = db.get_user(current_user.user_id)
    if existing:
        return existing
...
@app.get("/users/me/decks", response_model=List[Deck])
async def get_my_decks(
    db: DBService = Depends(get_db_service),
    current_user: CurrentUser = Depends(current_user_dependency),
):
    return db.get_user_decks(current_user.user_id)
...
async def get_my_stats(
    ...
    decks = db.get_user_decks(current_user.user_id)
    all_flashcards = []
    for deck in decks:
